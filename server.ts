...
async function startServer() {
  ...
  app.post("/api/consultar", async (req, res) => {
    ...
    try {
      // Fetching real company data from a public API
      const response = await axios.get(`https://publica.cnpj.ws/cnpj/${cleanCnpj}`);
      const data = response.data;

      // In a real scenario, we would redirect to government sites or scrape them.
      // Here we generate a professional "Certidão de Dados Cadastrais"
      res.json({ 
        success: true, 
        data: data,
